---
title: Compaction
llms-description: Compacting message histories for long-running agents that exceed the context window.
---
...
`CompactionAuto` is the recommended default for most use cases—it automatically uses native compaction when available and falls back to summary-based compaction otherwise. Edit and trim compaction are good for short or medium horizon tasks where you want to preserve as much context as possible.
...
`CompactionAuto` provides the best of both worlds: it uses efficient provider-native compaction when available and falls back to summary-based compaction for unsupported providers. This is the recommended default for most use cases.
...
react(
    tools=[bash(), text_editor()],
    compaction=CompactionAuto(threshold=0.9)
)
```
...
| `instructions` | None | Additional instructions to give the model about compaction (e.g. "Focus on preserving code snippets and technical decisions.") |
| `memory` | "auto" | Warn the model to save content to memory before compaction (when the memory tool is available). `"auto"` enables warnings for all compaction paths. |
...
Note that `CompactionNative` will raise `NotImplementedError` if the model provider doesn't support native compaction. Use `CompactionAuto` for automatic fallback to summary-based compaction.

For OpenAI Responses, `CompactionNative` and `CompactionAuto` also enable server-side compaction during generation. Inspect sends the strategy's resolved token threshold as `context_management=[{"type": "compaction", "compact_threshold": ...}]`, so long responses (including server-side web searches) can compact before returning. Fractional thresholds use the same model context-window calculation as between-turn compaction. An explicit `context_management` in `GenerateConfig.extra_body` overrides this automatic setting.

The dashboard displays compaction as it starts and completes, without requiring an `on_stream` callback. Completed encrypted compaction items are retained in eval logs and replayed on subsequent requests. The provider does not expose a readable summary. Existing between-turn compaction, including its `instructions` and memory warnings, remains in place; those options do not control OpenAI's inline compaction pass.

For the direct Anthropic API, these strategies also configure inline compaction on supported Claude models, using the resolved threshold and `instructions`. Thresholds below Anthropic's 50,000-token minimum retain between-turn compaction only. Explicit `GenerateConfig.extra_body.context_management` settings take precedence. The dashboard shows compaction progress followed by the readable summary, which is saved in eval logs and replayed on subsequent turns. Anthropic sends the completed summary in one delta rather than streaming its text token by token.
...
| `threshold` | 0.9 | Token count or percent of context window to trigger compaction. |
| `instructions` | None | Additional instructions to give the model about compaction (e.g. "Focus on preserving code snippets and technical decisions.") |
| `memory` | False | Warn the model to save content to memory before compaction (when the memory tool is available). Defaults to `False`. |
