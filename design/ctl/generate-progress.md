...
> **Status: implemented** (layer 1: activity indicator, including the `retry_wait` type per Open question 4; layers 2–3: progress channel + idle upgrade, merged upstream in [UKGovernmentBEIS/inspect_ai#4853](https://github.com/UKGovernmentBEIS/inspect_ai/pull/4853)). The layer-2 reporting path shipped as the shared model stream observer (`inspect_ai/model/_stream.py`), which also serves upstream #4186's public `on_stream` callback and #4268's live partial-output snapshots: providers report chunks once and the observer fans out to all three consumers, maintaining the pending event's progress record exactly as specified below. The layer-2 sections have been updated to describe the as-built shape (which supersedes the original `report_active_model_progress()` sketch). All providers in the table below are instrumented (Anthropic, Google, OpenAI, OpenAI-compatible/Together, Grok, SageMaker — see the as-shipped note after the table); the TUI caption (see "TUI adoption") is deferred. Companion to [`control-channel.md`](control-channel.md), which owns the control-channel architecture and documented this gap twice (the Phase 1 `GET /evals/<id>/samples` caveat, and §"Trace-log anomalies for stall diagnosis"); this doc owns closing it. Originating issues: meridianlabs-ai/inspect_ai#158 (layer 1) and meridianlabs-ai/inspect_ai#175 (layers 2–3).
...
- **Changing when providers stream for monitoring's sake.** The monitoring consumers instrument streams that already flow; they never turn streaming on by themselves (that would change provider code paths, response assembly, and failure modes for a display concern). An explicit `on_stream` callback is different — it *is* a request for deltas, so instrumented providers with an auto/unset streaming setting stream when one is passed (`model_stream_requested()` in `_stream.py`); an explicit provider-level streaming opt-out still wins.
...
- **Provider streaming existed but was consumed silently** (the pre-layer-2 baseline). Anthropic streams (auto, when thinking is on or `max_tokens >= 8192`) and iterated the event stream already (`_capture_compaction_from_stream`), discarding deltas; Google iterated chunks and accumulated; SageMaker hand-parses SSE including a usage chunk; Grok iterates and keeps only the last value; OpenAI-compatible calls `get_final_completion()` without observing chunks. No provider reported anything mid-flight — there was no chunk callback, no in-flight usage ContextVar, nothing. (Layer 2 closed this for Anthropic and Google; the rest are follow-up per the table below.)
...
2. **Progress channel** — a cheap per-chunk reporting path from provider streaming loops onto the pending event (shipped as the `report_model_stream_*` functions in `model/_stream.py`); providers that already iterate a stream report per delta, with cumulative output tokens where the provider supplies them.
...
Shipped alongside layer 1: the three places that documented the blind spot — the Phase 1 caveat in `control-channel.md`, `docs/control-channel.qmd`'s `sample list` section, and the comment in `_active_sample_summary` — now describe the activity indicator (with layers 2–3 noted, at the time, as the remaining streamed-progress work — the layer-3 update to `docs/control-channel.qmd` has since landed with #4853). Two implementation notes beyond the spec above: the retry-wait record lives on `ActiveSample` as a single slot (concurrent generates overwrite last-writer-wins; a per-context ownership guard keeps one call's clear from dropping a sibling's live wait — clearing on resolution rather than on next-attempt append matters because no next attempt ever appends after a final failure), and stamping is gated off for batch admin-op retry loops, whose worker task inherits an arbitrary sample's context.

### Layer 2 — the progress channel

**Reporting API (as built).** This design originally sketched a dedicated `report_active_model_progress()` alongside `report_active_sample_retry` in `src/inspect_ai/log/_samples.py`, resolving the pending event via the `_active_model_event` ContextVar. What shipped (upstream #4853) generalizes that: upstream #4186 (public `on_stream` streaming callbacks on `Model.generate()`) and #4268 (live partial output in inspect view) need the same per-chunk tap on the same provider loops, so the reporting path is a shared `ModelStreamObserver` (`src/inspect_ai/model/_stream.py`) that the model wrapper installs via its own ContextVar (`model_stream_observer()`) around each provider attempt. Provider loops report each chunk once through module-level functions and the observer fans out to all three consumers — the progress record, the throttled partial-output snapshot, and `on_stream`:

- `report_model_stream_progress(output_tokens=None)` — per chunk; the cumulative output token count for the current stream when the provider reports one, else `None` (a bare heartbeat). This *is* this design's progress channel, with the originally specified contract intact: a couple of attribute writes, cheap enough per chunk with no throttling, no lock per the repo's no-speculative-locks rule (single event loop; stream loops don't interleave mid-statement), and **no fabricated token estimates**.
- `report_model_stream_delta(delta)` — content deltas (text / reasoning / tool-call fragments); feeds the progress heartbeat plus the other two consumers. Delta reporting is gated on an `on_stream` handler being present (meridianlabs-ai/inspect_ai#350, after upstream #5061 showed a delta-construction bug failing calls that never passed a callback): provider loops consult `model_stream_requested()` per chunk and report a bare heartbeat instead when it is false, and `report_delta` itself backstops the reporting side of any ungated call site (delta construction can only be gated where it happens). Consequently the partial-output snapshot consumer — which is built from deltas — also runs only for `on_stream` callers; the progress record works for every streamed call via the heartbeat channel.
- `report_model_stream_start()` — at each stream open; rolls the cumulative-token base so a re-opened stream within one attempt (Anthropic `pause_turn` continuations, SDK-internal stream restarts) *adds to* the count rather than overwriting it, keeping the progress record monotonic across continuations.
- `report_model_stream_restart()` — a provider-internal retry that *regenerates* output within one attempt (Google's malformed-function-call retry): discards the accumulated partial output and resets the record's token count to null, since the old count described a discarded stream (counts are never estimated, so null until the replacement stream reports usage).

One observer spans a generate call's retry attempts; `begin_attempt` binds each attempt's fresh pending event and resets per-attempt state, so the wrapper — not providers — owns attempt boundaries and each attempt's progress record starts clean. The original design's concurrency property is unchanged: ContextVars are per-coroutine-context, so concurrent generates within one sample each hit their own observer and pending event. Every reporting function is a no-op when no observer is installed (e.g. provider-internal generates that don't run under the model wrapper). One nuance the observer added to the non-goals: an explicit `on_stream` callback *is* a request for deltas, so providers with an auto/unset streaming setting consult `model_stream_requested()` and stream when one is passed — the monitoring consumers alone still never turn streaming on.

**Where the record lives: on the pending event, not serialized.** The progress record (`ModelEventProgress`: `last_progress_at` wall-clock timestamp, `output_tokens` cumulative-when-known) is a small mutable object attached to the `ModelEvent` as the pydantic `PrivateAttr` `_progress`, exposed via the typed accessor `model_event_progress(event)` in `src/inspect_ai/event/_model.py` (which returns the record only while the event is pending), because `pending_events` is already the discovery path every consumer uses (control server, TUI, ACP) — attaching to the event needs no key management and is garbage-collected with the call. It must not be a serialized field: it is meaningful only while pending, and adding it to the event schema would leak an ephemeral monitoring artifact into logs and the type-generation pipeline (verified at ship time: `ModelEvent.model_json_schema()` unchanged, no ts-mono regeneration needed).

**Deliberately not `transcript()._event_updated()` per delta.** `_event_updated` fans out to subscribers, one of which persists to the realtime buffer DB (insert-only — each update is a new row). Routing per-chunk progress through it would turn a 15k-token stream into thousands of buffer inserts per call, exactly the class of cost the endpoint-cost-audit exists to prevent. Readers poll (the control server on request, the TUI on its 1-second tick); a poll of an in-memory record needs no notification. The consequence — progress is invisible to the buffer DB and to crash recovery — is correct: a recovered log should contain the completed call or its error, not a partial token count. The one deliberate, bounded exception lives beside the progress record in the same observer: the *partial-output snapshot* consumer (upstream #4268) does route through `_event_updated` so inspect view can render output growing live, but throttled to one flush per second (`PARTIAL_OUTPUT_FLUSH_INTERVAL`) — far below per-delta cost — and a failed or cancelled attempt discards its published snapshot so an errored event never carries partial output as if it were a real response. Being delta-driven, the snapshot runs only for `on_stream` callers (see the delta-gating note above). The progress record itself is notification-free exactly as specified here.

**Provider integration.** Instrument the loops that already exist; leave transport decisions alone:

| Provider | Loop exists? | Tokens available mid-stream? | Status |
|---|---|---|---|
| Anthropic | yes (`_capture_compaction_from_stream`) | yes — `message_delta` carries cumulative `usage.output_tokens` | **shipped** (#4853): text/thinking/tool-input deltas, tokens from `message_delta`, heartbeats for other events |
| Google | yes (chunk loop in `_stream_generate_content`) | chunks carry `usage_metadata` (cumulative where present) | **shipped** (#4853): per-chunk usage; first-candidate-only content deltas (avoids interleaving multi-candidate streams) |
| SageMaker | yes (manual SSE parse) | usage parsed already (final chunk) | follow-up: report per event |
| Grok | yes (`async for ... in chat.stream()`) | per SDK | follow-up: report per iteration |
| OpenAI-compatible / Together | no — `await stream.get_final_completion()` | usage only in final chunk (`stream_options.include_usage`) | follow-up: iterate the SDK stream's events (then `get_final_completion()` as today) reporting heartbeats; tokens stay null until the final chunk |
...
The provider work is independent per provider and lands incrementally; the surface degrades gracefully (null `tokens`, null `last_progress_at`) for any provider not yet instrumented. (As-shipped: all of the above plus the OpenAI provider — whose Responses/chat paths gained an `on_stream`-gated streaming mode for this purpose — and the OpenAI-compatible chat/responses paths are instrumented; each honors `model_stream_requested()` in its stream decision with an explicit provider-level opt-out winning. A follow-up round added `on_stream`-gated streaming modes to the four remaining direct-`ModelAPI` providers with streaming APIs: Bedrock (ConverseStream), Groq, Mistral (completions API path only — the default Conversation API path does not stream), and Azure AI, each with a hand-rolled accumulator that reconstructs the non-streaming response shape.)

### Layer 3 — idle means "time since last observed progress"

`_active_sample_summary` (`src/inspect_ai/_control/state.py`) computes `last_activity_at` as the max of the last transcript event's timestamp and every pending event's `last_progress_at`. The CLI's idle computation (`now - last_activity_at`) is unchanged — meaning **old CLIs against a new server get the corrected idle for free**, and the field's documented meaning shifts from "last transcript event" to "last observed activity" (`docs/control-channel.qmd` documents the new semantics).
...
- Progress channel: shipped as `tests/model/test_model_stream.py` — observer fan-out to all three consumers, per-attempt reset and retry boundaries (including eager `StreamRetryEvent` emission and cache-hit retries), provider continuations (`report_model_stream_start`) vs. regenerating restarts (`report_model_stream_restart`), `on_stream` detach-on-exception, and two concurrent generates in one sample; runs under both anyio backends.
- Provider loops: per-provider tests are thin (assert the loop calls the reporter — e.g. the Google chunk-loop tests in `tests/model/providers/test_google.py`); the recorded-response harnesses don't exercise real streams, so live-API smoke coverage rides the existing provider test tiers.
